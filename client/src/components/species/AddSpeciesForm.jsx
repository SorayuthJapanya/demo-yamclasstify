  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-4 relative py-2 mb-2">
        <img
          src={image ? URL.createObjectURL(image) : "/solid.png"}
          alt="Preview"
          className="size-50 rounded-xl object-cover"
        />
        <label
          htmlFor="image-upload"
          className="absolute right-43 bottom-8 cursor-pointer hover:bg-black duration-200 px-2 py-2 bg-black/70 rounded-full"
        >
          {!image ? <Camera className="size-5 text-white" /> : null}
        </label>
        {image && (
          <button
            type="button"
            className="cursor-pointer duration-200 px-2 py-2 rounded-full bg-red-500 absolute bottom-8 right-43 hover:bg-red-700"
      ...
      <button
        type="submit"
        className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 duration-200 cursor-pointer"
        disabled={isLoading}
      >
